...
pipeline{
    ...
    stages {
        ...
        stage('Checkout from Git'){
            steps{
                git branch: 'devsecops-cicd-project-one', url: 'YOUR_GITHUB_REPO_URL'  //update and provide your GitHub repo URL
            }
        }
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential'){
                       ...
                       sh "docker tag reddit ursulan1/reddit:latest "
                       sh "docker push ursulan1/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image ursulan1/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
